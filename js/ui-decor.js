...
function bugleFunction() {
    var nodes = document.getElementsByClassName('bugle-date');
    for (var i=0,ilen=nodes.length;i<ilen;i+=1) {
        var node = nodes[i];
        var pageDate = parseInt(node.textContent,10);
        var now = new Date();
        var nowDate = now.getTime();
        // Three days, counting in seconds
        if (pageDate > (nowDate - 4320)) {
