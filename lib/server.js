(function () {
    ...
    serverClass.prototype.runServer = function () {
        ...
        http.createServer(function (request, response) {
            ...
            if (sys.admin[params.admin]
                ...
                form.parse(request, function(err, fields, files) {
                    ...
                    function updateDocument(documentID) {
                        ...
                        fs.writeFileSync('style/' + sys.output_style + '/attachments/' + documentID,docTxt);
            ...
            function serveMe() {
                ...
                request.on('end', function(){
                    ...
                    console.log("TRY: "+uriObj.href);
                    
                    if (uriObj.href.match(/\/attachments\/[0-9]+$/)) {
                        var m = uriObj.href.match(/attachments\/([0-9]+)$/);
                        var documentID = m[1];
                        var docTxt = fs.readFileSync('style/' + sys.output_style + '/attachments/' + documentID);
                        ...
                        sys.db.get(sql,[documentID],function(err,row){
                            ...
                            if (row && row.title) {
                                ...
                                response.writeHead(200, {'Content-Type': row.mimeType,
                                                         'Content-Disposition': 'attachment; filename="' + row.title + filenameExtension + '"'
                                                        });
                                response.end(docTxt);
                            } else {
                        ...
                        console.log("READING FROM: "+__dirname + '/../' + uriObj.href.replace(/^.*?\/(css\/images\/.*)/, '$1'));
        ...
        }).listen(this.sys.newswriter_port);
    }
