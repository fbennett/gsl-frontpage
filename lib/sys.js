(function () {
    ...
    sysClass.prototype.getSys = function () {
        ...
        var pageEngine = function (sys) {
            ...
            this.outboundMap = {
                'Events/':sys.events_path,
                'Announcements/':sys.announcements_path,
                'events.atom':sys.event_feed_path,
                'announcements.atom':sys.announcement_feed_path,
                'calendar.ics':sys.calendar_path,
                'index.html':sys.top_page_path
            };
            ...
            if ("undefined" === typeof data.length || name === dSeg) {
                ...
                for (var i=0,ilen=data.length;i<ilen;i+=1) {
                    ...
                    if (data[i].eventID) {
                        ...
                        newBlock = this.substituteTouchDate(newBlock,cacheCode,data);
                    }
                    block += newBlock;
                }
            }
        ...
        pageEngine.prototype.substituteTouchDate = function(newBlock,cacheCode,data) {
            ...
            if (virt) {
                ...
                try {
                    this.sys.fs.readdirSync('cache');
                } catch (e) {
                    this.sys.fs.mkdirSync('cache');
                }
                var oldBlock = null;
                try {
                    oldBlock = this.sys.fs.readFileSync('cache/' + cacheCode).toString();
                } catch (e) {}
                ...
                if (!oldBlock || oldBlock !== newBlock) {
                    this.sys.fs.writeFileSync('cache/' + cacheCode,newBlock);
                }
                if (oldBlock && oldBlock == newBlock) {
